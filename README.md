...
[点击查看项目 Demo 视频](outputs/demo_video.mp4)
...
| 比赛要求 | 当前实现 |
| --- | --- |
| 指定场景真实对话训练 | 支持面试、点餐、会议三类角色扮演场景，AI 会主动开场并按阶段追问。 |
| 场景选择 | 左侧训练参数可切换场景和难度；目标模式会自动进入面试中等难度演示配置。 |
| 实时语音对话 | 支持浏览器英文听写、录音/音频上传转写、自动写入回答框、AI 追问浏览器朗读；浏览器听写可直接提交进入下一轮。 |
| 语音端到端流畅性和延迟 | 每轮回答记录识别耗时、AI 回复耗时和总延迟，右侧面板展示端到端延迟指标。 |
| 发音评测 | 本地演示版基于语音识别状态、回答时长、估算语速 WPM 和完整度给出 Pronunciation / Fluency 评分；未接入真实音素级评测时会明确标注为本地估算。 |
| 语法/表达纠错 | 本地规则覆盖常见错误；配置 API Key 后可使用 AI 生成更自然的纠错、表达优化和替代表达。 |
| 课后总结 | 完成 5 轮后可生成课后总结，并支持 Markdown / HTML 报告导出。 |
| 可量化反馈 | 五维评分、Coach 训练动作、历史记录、学习统计、趋势图和错误本共同形成可追踪反馈。 |
